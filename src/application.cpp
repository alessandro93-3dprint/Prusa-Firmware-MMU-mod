...
bool Application::CheckMsgs() {
    ...
    while (modules::serial::Available()) {
        uint8_t b = modules::serial::ConsumeByte();

        // Intercetta comandi “Tn”
        if (b == 'T') {
            if (!modules::serial::Available()) break;
            uint8_t digit = modules::serial::ConsumeByte();
            if (digit >= '0' && digit <= '9') {
                uint8_t slot = digit - '0';
                // **Qui inizializziamo correttamente rq**
                mp::RequestMsg rq(mp::RequestMsgCodes::Tool, slot);
                PlanCommand(rq);
                ReportCommandAccepted(rq, mp::ResponseMsgParamCodes::Accepted);
            }
            // scarta fino a fine linea
            while (modules::serial::Available() && modules::serial::ConsumeByte() != '\n')
                ;
            continue;
        }

        // Altrimenti procedi col parser standard
        switch (protocol.DecodeRequest(b)) {
